...
export async function POST(
  ...
  try {
    ...
    console.log("🚀 API admin/mandats/approve - Début pour mandat:", params.id);
    ...
    if (sessionError || !session?.user) {
      console.log("❌ Admin approve mandat: Session invalide");
      return NextResponse.json(
    ...
    if (!userEmail || !allowedEmails.includes(userEmail)) {
      console.log(`🚫 Admin approve mandat: Accès refusé pour ${userEmail}`);
      return NextResponse.json(
        {
          success: false,
    ...
    if (updateError) {
      console.error("❌ Erreur lors de l'approbation du mandat:", updateError);
      return NextResponse.json(
        {
          success: false,
    ...
    console.log(`✅ Mandat ${mandatId} approuvé par ${userEmail}`);
    ...
    try {
      ...
      if (creator && mandat.payload) {
        ...
        const emailData = EmailService.generateMandatApprovalEmail(
        ...
        console.log(`📧 Email de notification envoyé à ${creator.email}`);
      }
    } catch (emailError) {
    ...
    console.error("💥 Erreur admin approve mandat:", error);
    return NextResponse.json(
      {
        success: false,
